...
async function getTrackIdsByArtist(artistName) {
  try {
    ...
    const searchUrl = `https://v1.nocodeapi.com/mariopozio1/spotify/RQEyzKXgaFlBuXSo/search?q=${encodedArtistName}&type=track`;
...
async function myTop5(artistNames) {
   ...
   window.location.href = `searchpage#trackIds=${encodedTrackIds}`;
}
...
async function getArtistInfo(artistName) {
  try {
          const accessToken =  '';
    const searchUrl = `https://v1.nocodeapi.com/mariopozio1/spotify/RQEyzKXgaFlBuXSo/search?q=${encodeURIComponent(artistName)}&type=artist`;
